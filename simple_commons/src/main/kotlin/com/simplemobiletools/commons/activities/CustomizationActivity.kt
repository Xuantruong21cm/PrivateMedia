...
class CustomizationActivity : BaseSimpleActivity() {
    ...
    private lateinit var mBinding : ActivityCustomizationBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateMaterialActivityViews(mBinding.customizationCoordinator,mBinding.customizationHolder, useTransparentNavigation = true, useTopSearchMenu = false)
            ...
            ensureBackgroundThread {
                try {
                    ...
                    runOnUiThread {
                      ...
                      mBinding.applyToAllHolder.beVisibleIf(
        ...
        if (resources.getBoolean(R.bool.hide_google_relations) && !isThankYou) {
           mBinding.applyToAllHolder.beGone()
        }
    }
    ...
    override fun onResume() {
        ...
        setupToolbar(mBinding.customizationToolbar, NavigationIcon.Cross, getColoredMaterialStatusBarColor())
    }

    private fun refreshMenuItems() {
       mBinding.customizationToolbar.menu.findItem(R.id.save).isVisible = hasUnsavedChanges
    }

    private fun setupOptionsMenu() {
        mBinding.customizationToolbar.setOnMenuItemClickListener { menuItem ->
    ...
    private fun setupThemePicker() {
        ...
        mBinding.customizationTheme.text = getThemeText()
        updateAutoThemeFields()
        handleAccentColorLayout()
       mBinding.customizationThemeHolder.setOnClickListener {
        ...
        if (mBinding.customizationTheme.value == getMaterialYouString()) {
           mBinding.applyToAllHolder.beGone()
        }
    }
    ...
    private fun themePickerClicked() {
        ...
        RadioGroupDialog(this@CustomizationActivity, items, curSelectedThemeId) {
            ...
            updateMenuItemColors(mBinding.customizationToolbar.menu, getCurrentStatusBarColor())
            setupToolbar(mBinding.customizationToolbar, NavigationIcon.Cross, getCurrentStatusBarColor())
        }
    }
        ...
        resources.apply {
            if (curSelectedThemeId == THEME_CUSTOM) {
                if (useStored) {
                    ...
                    updateMenuItemColors(mBinding.customizationToolbar.menu, curPrimaryColor)
                    setupToolbar(mBinding.customizationToolbar, NavigationIcon.Cross, curPrimaryColor)
                    setupColorsPickers()
                } else {
    ...
    private fun updateAutoThemeFields() {
        arrayOf(mBinding.customizationTextColorHolder,mBinding.customizationBackgroundColorHolder).forEach {
            it.beVisibleIf(curSelectedThemeId != THEME_AUTO && curSelectedThemeId != THEME_SYSTEM)
        }

       mBinding.customizationPrimaryColorHolder.beVisibleIf(curSelectedThemeId != THEME_SYSTEM)
    }
    ...
    private fun setupColorsPickers() {
       ...
       mBinding.customizationTextColor.setFillWithStroke(textColor, backgroundColor)
       mBinding.customizationPrimaryColor.setFillWithStroke(primaryColor, backgroundColor)
       mBinding.customizationAccentColor.setFillWithStroke(curAccentColor, backgroundColor)
       mBinding.customizationBackgroundColor.setFillWithStroke(backgroundColor, backgroundColor)
       mBinding.customizationAppIconColor .setFillWithStroke(curAppIconColor, backgroundColor)
       mBinding.applyToAll.setTextColor(primaryColor.getContrastColor())

       mBinding.customizationTextColorHolder.setOnClickListener { pickTextColor() }
       mBinding.customizationBackgroundColorHolder.setOnClickListener { pickBackgroundColor() }
       mBinding.customizationPrimaryColorHolder.setOnClickListener { pickPrimaryColor() }
       mBinding.customizationAccentColorHolder.setOnClickListener { pickAccentColor() }

        handleAccentColorLayout()
        mBinding.applyToAll.setOnClickListener {
            applyToAll()
        }

       mBinding.customizationAppIconColorHolder.setOnClickListener {
            if (baseConfig.wasAppIconCustomizationWarningShown) {
                pickAppIconColor()
            } else {
                ConfirmationDialog(this, "", R.string.app_icon_color_warning, R.string.ok, 0) {
    ...
    private fun updateApplyToAllColors(newColor: Int) {
        if (newColor == baseConfig.primaryColor && !baseConfig.isUsingSystemTheme) {
           mBinding.applyToAll.setBackgroundResource(R.drawable.button_background_rounded)
        } else {
        ...
        mBinding.applyToAll   .background = applyBackground
        }
    }

    private fun handleAccentColorLayout() {
      mBinding.customizationAccentColorHolder.beVisibleIf(curSelectedThemeId == THEME_WHITE || isCurrentWhiteTheme() || curSelectedThemeId == THEME_BLACK_WHITE || isCurrentBlackAndWhiteTheme())
       mBinding.customizationAccentColorLabel.text = getString(
    ...
    private fun pickPrimaryColor() {
        ...
        curPrimaryLineColorPicker = LineColorPickerDialog(this, curPrimaryColor, true, toolbar =mBinding.customizationToolbar) { wasPositivePressed, color ->
            ...
            if (wasPositivePressed) {
                ...
                updateMenuItemColors(mBinding.customizationToolbar.menu, color)
                setupToolbar(mBinding.customizationToolbar, NavigationIcon.Cross, color)
            } else {
                updateActionbarColor(curPrimaryColor)
                setTheme(getThemeId(curPrimaryColor))
                updateMenuItemColors(mBinding.customizationToolbar.menu, curPrimaryColor)
                setupToolbar(mBinding.customizationToolbar, NavigationIcon.Cross, curPrimaryColor)
                updateTopBarColors(mBinding.customizationToolbar, curPrimaryColor)
            }
        }
    }
    ...
    private fun updateLabelColors(textColor: Int) {
        arrayListOf<MyTextView>(
           mBinding.customizationThemeLabel,
           mBinding.customizationTheme,
           mBinding.customizationTextColorLabel,
           mBinding.customizationBackgroundColorLabel,
           mBinding.customizationPrimaryColorLabel,
           mBinding.customizationAccentColorLabel,
           mBinding.customizationAppIconColorLabel
        ).forEach {
    ...
    private fun getCurrentTextColor() = if (mBinding.customizationTheme.value == getMaterialYouString()) {
        resources.getColor(R.color.you_neutral_text_color)
    } else {
        curTextColor
    }

    private fun getCurrentBackgroundColor() = if (mBinding.customizationTheme.value == getMaterialYouString()) {
        resources.getColor(R.color.you_background_color)
    } else {
        curBackgroundColor
    }

    private fun getCurrentPrimaryColor() = if (mBinding.customizationTheme.value == getMaterialYouString()) {
        resources.getColor(R.color.you_primary_color)
    } else {
        curPrimaryColor
    }

    private fun getCurrentStatusBarColor() = if (mBinding.customizationTheme.value == getMaterialYouString()) {
