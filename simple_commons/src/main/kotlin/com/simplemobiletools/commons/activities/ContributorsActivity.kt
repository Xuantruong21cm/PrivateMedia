...
class ContributorsActivity : BaseSimpleActivity() {
    ...
    private lateinit var mBinding : ActivityContributorsBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        isMaterialActivity = true
        super.onCreate(savedInstanceState)

        mBinding = ActivityContributorsBinding.inflate(layoutInflater)

        setContentView(mBinding.root)
        updateTextColors(mBinding.contributorsHolder)

        updateMaterialActivityViews(mBinding.contributorsCoordinator,mBinding.contributorsHolder, useTransparentNavigation = true, useTopSearchMenu = false)
        setupMaterialScrollListener(mBinding.contributorsNestedScrollview,mBinding.contributorsToolbar)

        val primaryColor = getProperPrimaryColor()
        mBinding.contributorsDevelopmentLabel.setTextColor(primaryColor)
        mBinding.contributorsTranslationLabel.setTextColor(primaryColor)
        ...
        languages.forEach { language ->
            inflater.inflate(R.layout.item_language_contributor, null).apply {
                findViewById<ImageView>(R.id.language_icon).setImageDrawable(getDrawable(language.iconId))
               findViewById<TextView>(R.id.language_label).apply {
                    text = getString(language.labelId)
                    setTextColor(textColor)
                }

               findViewById<TextView>(R.id.language_contributors).apply {
                    text = getString(language.contributorsId)
                    setTextColor(textColor)
                }

               mBinding.contributorsLanguagesHolder.addView(this)
            }
        }

       mBinding.contributorsLabel.apply {
            setTextColor(textColor)
       ...
       mBinding.contributorsDevelopmentIcon.applyColorFilter(textColor)
       mBinding.contributorsFooterIcon.applyColorFilter(textColor)

        if (resources.getBoolean(R.bool.hide_all_external_links)) {
            mBinding.contributorsFooterLayout.beGone()
        }
    }

    override fun onResume() {
        super.onResume()
        setupToolbar(mBinding.contributorsToolbar, NavigationIcon.Arrow)
    }
}
