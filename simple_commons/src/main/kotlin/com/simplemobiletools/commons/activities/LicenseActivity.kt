...
class LicenseActivity : BaseSimpleActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding = ActivityLicenseBinding.inflate(layoutInflater)

        setContentView(mBinding.root)
        updateTextColors(mBinding.licensesHolder)

        updateMaterialActivityViews(mBinding.licensesCoordinator,mBinding.licensesHolder, useTransparentNavigation = true, useTopSearchMenu = false)
        setupMaterialScrollListener(mBinding.licensesNestedScrollview,mBinding.licensesToolbar)
        ...
        licenses.filter { licenseMask and it.id != 0L }.forEach {
            ...
            inflater.inflate(R.layout.item_license, null).apply {
                findViewById<MaterialCardView>(R.id.license_card).setCardBackgroundColor(backgroundColor)
               findViewById<MyTextView>(R.id.license_title).apply {
               ...
               findViewById<MyTextView>(R.id.license_text).apply {
                    text = getString(license.textId)
                    setTextColor(textColor)
                }

               mBinding.licensesHolder.addView(this)
            }
        }
    }
    ...
    override fun onResume() {
        super.onResume()
        setupToolbar(mBinding.licensesToolbar, NavigationIcon.Arrow)
    }
